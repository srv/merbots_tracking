...
void TargetTracker::run()
{
    ...
    while (ros::ok())
    {
        ...
        if (sdata->getStatus() == TRACKING && sdata->existsImage() && sdata->existsTarget())
        {
            ...
            // Image to get points from
            cv::Mat img_points;

            // If ROI fits on window
                ...
                (curr_roi.y > 0 && (curr_roi.y + curr_roi.height) < image.rows))
            {
                ...
                // --- Image should be restricted to the ROI area ---
            ...
            if (!change_stat)
            {
                ...
                p->det_detector->detect(img_points, img_kps);
                cv::Mat img_descs;
                p->det_descriptor->compute(img_points, img_kps, img_descs);
