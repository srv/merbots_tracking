...
TrackerStruck::TrackerStruck(const Config& conf) :
	m_config(conf),
	m_initialised(false),
	m_pLearner(0),
...
void TrackerStruck::Initialise(const cv::Mat& frame, FloatRect bb)
{
	m_bb = IntRect(bb);
...
void TrackerStruck::Track(const cv::Mat& frame)
{
...
void TrackerStruck::UpdateDebugImage(const vector<FloatRect>& samples, const FloatRect& centre, const vector<double>& scores)
{
